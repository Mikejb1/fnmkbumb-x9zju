...
const AI_WORKER_URL = PORTFOLIO_CONFIG.workerUrl || 'https://morning-tree-9bb.michael-bummer.workers.dev';
const USER_KEY = PORTFOLIO_CONFIG.userKey;
const ACCOUNT_LABEL = PORTFOLIO_CONFIG.accountLabel || USER_KEY;
if (!USER_KEY || !/^[a-z0-9_-]{1,32}$/i.test(USER_KEY)) { throw new Error('PORTFOLIO_CONFIG.userKey fehlt oder ist ungueltig'); }
...
const LAYOUT_SECTIONS = [
  ...
  { key: 'positions', label: 'Positionen', meta: 'Aktien, ETF, Krypto, Edelmetalle, Cash', selectors: ['#anchor-positions', '#positions'], nav: 'anchor-positions' },
...
function renderLayoutModal(settingsOverride) {
  ...
  if (positionList) {
    const positions = Array.isArray(appData?.positions) ? appData.positions : [];
    positionList.innerHTML = positions.length
      ? positions.map(positionLayoutRowHtml).join('')
      : '<div class="layout-meta" style="padding:8px;">Noch keine Titel vorhanden.</div>';
...
function buildFactSheet(totals, goal) {
  ...
  if (goal.amount && goal.year) {
    ...
    const endOfGoal = new Date(goal.year, 11, 31).getTime();
    const now = Date.now();
    const elapsed = (now - startOfYear) / (endOfGoal - startOfYear);
    const expectedProgress = goal.amount * elapsed;
...
- Ziel: ${fmt.format(goal.amount)} bis ${goal.year} · Status: ${goalStatus}
...
const BAKED_BLOB = PORTFOLIO_CONFIG.bakedBlob || null;
...
let appAuthToken = null;       // wird beim Login aus dem Master-Code abgeleitet; kein statisches Frontend-Geheimnis
...
async function deriveUserKey(masterCode) {
  try {
    ...
    return USER_KEY; // Notfall-Fallback nur fuer Legacy-Migration
  }
}
...
function getPositionValuation(pos, liveOverride) {
  ...
  if (pos.special === 'cash') {
    const value = Number(appData?.goal?.cash || pos.manualPrice || pos.costPrice || 0);
...
// Zeitgewichtete Rendite (Time-Weighted Return / TWR) auf Basis täglicher Werte und Cashflows
// Erwartet `points` aus buildPortfolioHistory mit value + invested (täglich). Cashflows werden aus Tx-Tagen extrahiert.
function computeTWR(points) {
  if (!points || points.length < 2) return null;
  // Cashflows je Datum: positive = Einzahlung/Buy (Geld kommt rein), Buys = +value
  const cashflowsByDate = {};
  (appData?.transactions || []).forEach(t => {
    if (t.assetType === 'cash') return; // Einzahlungen sind Externals, nicht relevant für TWR der Investments
    const k = t.date;
    if (!k) return;
    if (t.txType === 'buy') cashflowsByDate[k] = (cashflowsByDate[k] || 0) + (t.value + (t.fees || 0));
    else if (t.txType === 'sell') cashflowsByDate[k] = (cashflowsByDate[k] || 0) - (t.value - (t.fees || 0));
  });
  // Segmentrendite: r_i = (V_i - CF_i) / V_{i-1}
  let product = 1;
  let validSegments = 0;
  for (let i = 1; i < points.length; i++) {
    const dateKey = points[i].date.toISOString().slice(0, 10);
    const cf = cashflowsByDate[dateKey] || 0;
    const prev = points[i - 1].value;
    const cur = points[i].value;
    if (prev <= 0) continue;
    const segRet = (cur - cf) / prev;
    if (!isFinite(segRet) || segRet <= 0) continue;
    product *= segRet;
    validSegments++;
  }
...
function sharesAtDate(pos, date) {
  ...
  if (Array.isArray(appData?.transactions) && appData.transactions.length > 0) {
    ...
    const txs = appData.transactions.filter(t => t.assetId === pos.id && (t.date || '') <= cutoff);
    if (txs.length > 0) {
      let shares = 0;
...
function buildStammdatenSection(pos, live, totals) {
  ...
  const rows = [
    ...
    row('Einstandskurs', fmt.format(valuation.costPrice)),
...
function metalGramsAtDate(metal, date) {
  ...
  if (Array.isArray(appData?.transactions) && appData.transactions.length > 0) {
    ...
    const txs = appData.transactions.filter(t => t.assetId === assetId && (t.date || '') <= cutoff);
    if (txs.length > 0) {
      let grams = 0;
      txs.forEach(t => {
...
function computeDataQuality(totals) {
  ...
  if (positions.length === 0) {
    ...
    const complete = positions.filter(p => p.shares > 0 && p.costPrice > 0).length;
    score += (complete / positions.length) * 25;
    const missing = positions.length - complete;
    if (missing > 0) issues.push({ severity: 'warn', text: `Bei ${missing} Position${missing === 1 ? '' : 'en'} fehlen Stückzahl oder Einstandspreis.` });
  }

  // 2) Kauf-Lots mit Datum und Betrag (25 %) — Anzahl Positionen mit ≥ 1 zugeordneter buy-Transaktion
  if (positions.length > 0) {
    const txs = appData?.transactions || [];
    const posWithTx = positions.filter(p => txs.some(t => t.assetId === p.id && t.txType === 'buy' && t.date && t.value > 0)).length;
    score += (posWithTx / positions.length) * 25;
    const noLot = positions.length - posWithTx;
    if (noLot > 0) issues.push({ severity: 'warn', text: `Bei ${noLot} Position${noLot === 1 ? '' : 'en'} fehlen Kaufdaten (Datum + Betrag).` });
  }
  ...
  if (hasCashTx) {
    ...
    score += 5;
    issues.push({ severity: 'info', text: 'Cash-Bewegungen sind nicht einzeln gepflegt (nur Slider-Wert).' });
  } else {
    score += 5;
  }
  ...
  if (positions.length > 0) {
    ...
    positions.forEach(p => {
      ...
      fields.forEach(f => { totalFields++; if (sd[f] && String(sd[f]).trim()) filledFields++; });
    });
    const stammRatio = totalFields > 0 ? filledFields / totalFields : 0;
    score += stammRatio * 15;
    const posWithoutAny = positions.filter(p => {
      const sd = p.stammdaten || {};
      return !fields.some(f => sd[f] && String(sd[f]).trim());
    }).length;
    if (posWithoutAny > 0) issues.push({ severity: 'info', text: `Bei ${posWithoutAny} Position${posWithoutAny === 1 ? '' : 'en'} fehlen Stammdaten (ISIN/WKN/Sektor).` });
  }
  ...
  if (goalPts < 10) {
    ...
    if (miss.length > 0) issues.push({ severity: 'info', text: `Ziel-Eingaben unvollständig (${miss.join(', ')}).` });
  }

  // 6) Backup vorhanden / aktuell (10 %)
  let backupTs = 0;
  try { backupTs = parseInt(localStorage.getItem(BACKUP_TS_KEY) || '0', 10); } catch (e) {}
  if (backupTs > 0) {
    const ageDays = (Date.now() - backupTs) / (1000 * 60 * 60 * 24);
    if (ageDays < 30) score += 10;
    else if (ageDays < 90) { score += 5; issues.push({ severity: 'warn', text: `Letztes Backup ist ${Math.floor(ageDays)} Tage alt — neues JSON-Backup empfohlen.` }); }
    else { score += 2; issues.push({ severity: 'warn', text: `Backup älter als 3 Monate — bitte aktuelles JSON-Backup herunterladen.` }); }
  } else {
    issues.push({ severity: 'warn', text: 'Noch kein lokales Backup heruntergeladen — JSON-Backup im Backup-Bereich nutzen.' });
  }
  ...
  if (issues.length === 0) issues.push({ severity: 'good', text: 'Alle wichtigen Daten sind gepflegt — Auswertungen sind belastbar.' });
...
function renderDataQuality(totals) {
  ...
  const { score, issues } = computeDataQuality(totals);
  ...
  issuesEl.innerHTML = issues.map(i => {
    ...
    return `<div class="quality-issue ${i.severity}"><span class="ico">${icoMap[i.severity] || '·'}</span><span>${escapeHtml(i.text)}</span></div>`;
  }).join('');
}
...
function openBuyMoreModal(posId) {
  ...
  document.getElementById('buyMoreSubtitle').textContent = `Aktuell ${fmtNum(pos.shares, pos.shares % 1 === 0 ? 0 : 4)} Stk @ Ø ${fmtNum(pos.costPrice)} € · letzter Kurs ${fmtNum(live.price)} €`;
  document.getElementById('bmDate').value = new Date().toISOString().slice(0, 10);
  document.getElementById('bmShares').value = '';
  document.getElementById('bmPrice').value = live.price.toFixed(2);
...
function updateBuyMorePreview() {
  ...
  body.innerHTML = `
    ...
    <div class="realized-line"><span class="lbl">Ø Einstand neu</span><span>${fmtNum(newAvg)} € (vorher ${fmtNum(pos.costPrice)} €)</span></div>
...
function openSellModal(posId) {
  ...
  const live = currentPrices[posId] || { price: pos.costPrice };
  document.getElementById('sellSubtitle').textContent = `Aktuell ${fmtNum(pos.shares, pos.shares % 1 === 0 ? 0 : 4)} Stk im Bestand · letzter Kurs ${fmtNum(live.price)} €`;
  ...
  document.getElementById('sellPrice').value = live.price.toFixed(2);
...
function updateSellPreview() {
  ...
  body.innerHTML = `
    ...
    <div class="realized-line"><span class="lbl">Einstand anteilig (Avg ${fmtNum(avgBefore)} €/Stk)</span><span>${fmt.format(costAnteilig)}</span></div>
...
async function saveIncomeModal() {
  ...
  if (editingIncomeId) {
    ...
    if (idx >= 0) list[idx] = { ...list[idx], date, positionId, kind, gross, tax: tax || 0, net, note };
  } else {
    list.push({ id: 'inc_' + Date.now().toString(36) + '_' + Math.random().toString(36).slice(2, 6), date, positionId, kind, gross, tax: tax || 0, net, note });
  }
...
function validateBackupSchema(data) {
  ...
  if (Array.isArray(data.positions)) {
    data.positions.forEach((p, i) => {
      if (!p.id || typeof p.id !== 'string') throw new Error(`Position ${i + 1}: id fehlt oder ungültig`);
      if (!p.name || typeof p.name !== 'string') throw new Error(`Position ${i + 1}: name fehlt`);
      if (p.shares != null && (typeof p.shares !== 'number' || !isFinite(p.shares))) throw new Error(`Position "${p.name}": shares ungültig`);
      if (p.costPrice != null && (typeof p.costPrice !== 'number' || !isFinite(p.costPrice))) throw new Error(`Position "${p.name}": costPrice ungültig`);
    });
  }
  // Transaktionen prüfen
  if (Array.isArray(data.transactions)) {
    const validTxTypes = new Set(['buy', 'sell', 'deposit', 'withdraw', 'fee', 'dividend', 'adjust']);
...
function quoteFreshnessInfo(pos, live) {
  ...
  if (!live?.live) {
    return {
      className: 'manual',
      shortText: `Quelle: ${source} · kein Live-Zeitstempel`,
  ...
  if (!d || isNaN(d.getTime())) {
    return {
      className: '',
      shortText: `Quelle: ${source} · ${venue} · Kurszeit: gerade geladen`,
  ...
  const nextMs = Math.max(0, d.getTime() + cadence * 60000 - now.getTime());
  const nextMin = Math.ceil(nextMs / 60000);
  const time = d.toLocaleTimeString('de-AT', { hour: '2-digit', minute: '2-digit' });
  const delayedText = ageMin <= 0 ? 'unter 1 min' : `ca. ${ageMin} min`;
  const nextText = nextMin <= 0 ? 'ca. 0 min' : `ca. ${nextMin} min`;
  const className = ageMin >= cadence ? 'stale' : '';
  const base = `Quelle: ${source} · ${venue} · Kurszeit: ${time} · verzögert ${delayedText} · nächster Wert in ${nextText}`;
  return { className, shortText: base, detailText: `${base} · angenommener Quellentakt: ${cadence} min` };
}
  ...
  wrap.innerHTML = `
    ...
    <div class="card-expanded-content">
      <div class="price-grid">
        <div class="price-grid-cell"><div class="lbl">Ø Einstieg</div><div class="val">${fmtNum(valuation.costPrice)} €</div></div>
        ...
        <div class="price-grid-cell"><div class="lbl">Break-Even</div><div class="val">${fmtNum(valuation.costPrice)} €</div></div>
...
function buildManualQuotePanel(pos, live) {
  ...
  const placeholder = basePrice ? fmtNum(basePrice, basePrice < 10 ? 4 : 2) : fmtNum(live?.price || pos.manualPrice || pos.costPrice || 0, 2);
  return `<div class="manual-quote-panel" data-manual-quote-panel="${pos.id}">
    <div class="manual-quote-head">
      ...
      <div class="manual-quote-state ${active ? 'active' : ''}" id="manualQuoteState-${pos.id}">
        ${active ? `Manueller Kurs ${fmtNum(active.price, active.price < 10 ? 4 : 2)} € · ${escapeHtml(remaining)}` : 'Inaktiv · Berechnung nutzt Live/Quelle'}
      </div>
    </div>
...
function renderVenueList(pos) {
  ...
  const currentPrice = live && live.price ? fmtNum(live.price) + ' €' : null;
...
async function saveCashTxModal() {
  ...
  if (!Array.isArray(appData.transactions)) appData.transactions = [];
  // Erste Cash-Tx? Falls ja: automatische Initial-Einzahlung anlegen, damit computed Cash sinnvoll startet
  // (sonst würde getCashBalance plötzlich auf -[Summe der Käufe] fallen)
  const hasExistingCashTx = appData.transactions.some(t => t.assetType === 'cash');
  if (!hasExistingCashTx) {
    const allBuys = appData.transactions
      .filter(t => t.assetType !== 'cash' && t.txType === 'buy')
      .reduce((s, t) => s + t.value + (t.fees || 0), 0);
    const allSells = appData.transactions
      .filter(t => t.assetType !== 'cash' && t.txType === 'sell')
      .reduce((s, t) => s + (t.value - (t.fees || 0)), 0);
    const sliderCash = Number(appData.goal?.cash || 0);
    const initialDeposit = allBuys - allSells + sliderCash;
    if (initialDeposit > 0) {
      const earliest = appData.transactions
        .filter(t => t.date).map(t => t.date).sort()[0] || date;
      appData.transactions.push({
        id: makeTxId(), date: earliest,
        assetId: 'cash', assetType: 'cash', txType: 'deposit',
  ...
  if (kind === 'metals') {
    ...
    const sliderHtml = hasCashTx ? `
      <div class="special-slider-wrap" style="opacity:0.6;">
        ...
        <input type="range" min="0" max="1000000" step="100" value="${cash}" disabled>
    ...
    ` : `
      <div class="special-slider-wrap">
        <span class="special-slider-min">0 €</span>
        <input type="range" id="cashSlider" min="0" max="1000000" step="100" value="${cash}">
...
function purchaseEventsForDate(date) {
  ...
  if (Array.isArray(appData?.transactions) && appData.transactions.length > 0) {
    const dayTxs = appData.transactions.filter(t => t.date === iso);
    dayTxs.forEach(t => {
...
function buildPortfolioHistory(period) {
  ...
  const currentTotals = (() => {
    ...
    appData.positions.forEach(pos => {
      const live = currentPrices[pos.id] || { price: pos.costPrice };
      t += getPositionValuation(pos, live).currentValue;
    });
    return t + extraToday;
  })();
  const points = [];
...
function renderHistory() {
  ...
  if (twrEl) {
    ...
    if (twr != null && isFinite(twr)) {
      const pct = twr * 100;
      twrEl.textContent = `${pct >= 0 ? '+' : ''}${fmtNum(pct, 1)} % p.a.`;
      twrEl.className = pct >= 0 ? 'positive' : 'negative';
    } else { twrEl.textContent = '—'; }
  }
...
function renderGoal(totals) {
  ...
  if (strategyEl) {
    strategyEl.innerHTML = `<strong>Strategie:</strong> ${strategy.pathLabel} (${fmtNum(strategy.savingsPct, 0)}% Sparrate / ${fmtNum(strategy.returnPct, 0)}% Rendite) · Risiko <strong>${strategy.riskLabel}</strong> (${fmtNum(strategy.riskPct, 0)}%). ${strategy.pathText}`;
  }
  if (appData.goal) {
  ...
  return { year, amount, gap, monthsToGoal, monthlyNeeded, requiredReturnPct, progress, savingsRate, ...strategy };
}
...
function renderAnalysisLocal(totals, goal) {
  ...
  document.getElementById('analysisText').innerHTML = `
    ...
    <div class="block"><strong>Strategie:</strong> ${strategy.pathLabel}: <strong>${fmtNum(strategy.savingsPct, 0)}% Sparrate / ${fmtNum(strategy.returnPct, 0)}% Rendite</strong>. Risiko: <strong>${strategy.riskLabel}</strong> (${fmtNum(strategy.riskPct, 0)}%).</div>
...
function openEditModal(posId) {
  ...
  const live = currentPrices[posId] || { price: pos.costPrice };
  document.getElementById('editPrice').value = live.price.toFixed(2);
...
async function saveEditModal() {
  ...
  if (sharesDiff || costDiff) {
    ...
    appData.transactions.push({
      ...
      note: 'Manuelle Korrektur via Edit-Modal' + (sharesDiff ? ` · Stück ${pos.shares} → ${newShares}` : '') + (costDiff ? ` · Ø ${fmtNum(pos.costPrice)} → ${fmtNum(newCost)} €` : '')
    });
    appData.transactions.sort((a, b) => (a.date || '').localeCompare(b.date || ''));
  }
  pos.shares = newShares;
...
function renderSavingsSim(totals, goal) {
  ...
  const rows = rates.map(rate => {
    ...
    const diff = projected - goal.amount;
    let cls, tag;
    if (projected < goal.amount * 0.85) { cls = 'late'; tag = `fehlt ${fmtNoCent.format(Math.abs(diff))}`; }
    else if (projected < goal.amount) { cls = 'tight'; tag = `fehlt ${fmtNoCent.format(Math.abs(diff))}`; }
    else if (projected < goal.amount * 1.1) { cls = 'fit'; tag = `Puffer ${fmtNoCent.format(diff)}`; }
  ...
  grid.innerHTML = rows;
  const currentProjected = totals.totalCur + current * months;
  const currentGap = goal.amount - currentProjected;
  if (currentGap <= 0) {
    note.innerHTML = `Aktuell eingestellt: <strong>${fmtNoCent.format(current)}/Mo</strong>. Damit landest du rechnerisch bei <strong>${fmt.format(currentProjected)}</strong> und hast einen Puffer von <strong style="color:var(--positive);">${fmt.format(-currentGap)}</strong>.`;
  } else {
    note.innerHTML = `Aktuell eingestellt: <strong>${fmtNoCent.format(current)}/Mo</strong>. Ohne Rendite würdest du <strong>${fmt.format(currentProjected)}</strong> erreichen; offen bleiben <strong style="color:var(--negative);">${fmt.format(currentGap)}</strong>. Rechnerisch nötig wären etwa <strong>${fmt.format(required)}/Mo</strong>.`;
  }
}
...
async function applyGoalSettingsUpdate() {
  ...
  try {
    ...
    await Promise.all([fetchCryptoPrices(), fetchMarketPrices({ forceRefresh: true }), fetchMetalPrices(), fetchMetalHistory(365)]);
    await fetchAllWeeklyCharts();
...
async function initApp() {
  ...
  await refreshUI({ skipAI: true });
  await Promise.all([fetchCryptoPrices(), fetchMarketPrices(), fetchMetalPrices(), fetchMetalHistory(365)]);
  await fetchAllWeeklyCharts();
  await refreshUI();
}
...
function wireEvents() {
  ...
  ['goalYear', 'goalAmount', 'goalSavings', 'goalPath', 'goalRisk'].forEach(id => {
  ...
  document.getElementById('positions').addEventListener('click', e => {
    ...
    if (e.target.closest('input, button, a, select, details, summary')) return;
...
function buildChatSystemPrompt() {
  ...
  const positions = getAllPositions().map(pos => {
    ...
    const pnlPct = valuation.pnlPct;
    return `- ${pos.name} (${pos.type}, ${pos.symbol || ''}): ${valuation.shares} Stk × ${live.price.toFixed(2)} € = ${value.toFixed(2)} € (${pct.toFixed(1)}% Allokation, P&L ${pnlPct >= 0 ? '+' : ''}${pnlPct.toFixed(1)}%)`;
  }).join('\n');
  ...
  const endDate = new Date(goal.year, 11, 31);
  const monthsToGoal = Math.max(1, Math.round((endDate - today) / (1000 * 60 * 60 * 24 * 30.44)));
  const goalEnriched = { ...goal, ...strategy, monthsToGoal, gap: goal.amount - totalCur, monthlyNeeded: (goal.amount - totalCur) / monthsToGoal, requiredReturnPct: totalCur > 0 ? ((goal.amount / totalCur) - 1) * 100 : 0 };
...
function openScreenshotPreviewModal(data) {
  ...
  if (data.manualPrice && data.costPrice) {
    ...
    if (Math.abs(diffPct) > 80) {
      items.push({ kind: 'warn', text: `Großer Unterschied zwischen Kurs (${fmtNum(data.manualPrice)} €) und Einstand (${fmtNum(data.costPrice)} €) — ${fmtNum(diffPct, 0)} %. Bitte verifizieren.` });
    }
  }
  ...
  if (data.costPrice && data.costPrice > 100000) {
    items.push({ kind: 'warn', text: `Sehr hoher Einstandspreis (${fmtNum(data.costPrice)} €). Bitte verifizieren.` });
  }
  if (items.length === 0) {
  ...
  batchEl.innerHTML = rows.map(row => {
    ...
    return `<div class="ss-batch-row ${row.txType === 'ignore' ? 'ignore' : (action === 'VERK.' ? 'sell' : 'buy')}">
      ...
      <div>
        ...
        <div class="meta">${escapeHtml(row.symbol || row.isin || '')} · ${fmtNum(row.quantity, row.quantity % 1 ? 6 : 0)} Stk · ${fmtNum(row.price)} € · ${fmt.format(row.amount || row.quantity * row.price)}</div>
